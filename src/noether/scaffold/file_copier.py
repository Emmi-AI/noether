...
def copy_python_files(config: ScaffoldConfig) -> None:
    ...
    _copy_template_with_substitution(tpl / "model" / "base.py", project_dir / "model" / "base.py", config)
    _copy_template_with_substitution(
    ...
    _copy_template_with_substitution(tpl / "pipeline" / "__init__.py", project_dir / "pipeline" / "__init__.py", config)
    _copy_template_with_substitution(
    ...
    _copy_template_with_substitution(
        tpl / "schemas" / "config_schema.py", project_dir / "schemas" / "config_schema.py", config
    )

    # --- Static init files (verbatim copies) ---
    _copy_verbatim(tpl / "callbacks" / "__init__.py", project_dir / "callbacks" / "__init__.py")
    _copy_verbatim(tpl / "trainers" / "__init__.py", project_dir / "trainers" / "__init__.py")
    _copy_verbatim(tpl / "schemas" / "__init__.py", project_dir / "schemas" / "__init__.py")
    _copy_verbatim(tpl / "schemas" / "datasets" / "__init__.py", project_dir / "schemas" / "datasets" / "__init__.py")
    _copy_verbatim(tpl / "schemas" / "pipelines" / "__init__.py", project_dir / "schemas" / "pipelines" / "__init__.py")
    _copy_verbatim(tpl / "schemas" / "trainers" / "__init__.py", project_dir / "schemas" / "trainers" / "__init__.py")
    _copy_verbatim(tpl / "schemas" / "callbacks" / "__init__.py", project_dir / "schemas" / "callbacks" / "__init__.py")
...
def copy_yaml_configs(config: ScaffoldConfig) -> None:
    ...
    for subdir, filename in verbatim:
        if filename:
            _copy_verbatim(tpl / subdir / f"{filename}.yaml", dst / subdir / f"{filename}.yaml")
    ...
    _copy_template_with_substitution(
        tpl / "model" / f"{config.model.value}.yaml", dst / "model" / f"{config.model.value}.yaml", config
    )

    pipeline_file = ref.get("pipeline_file")
    if pipeline_file:
        _copy_template_with_substitution(
            tpl / "pipeline" / f"{pipeline_file}.yaml", dst / "pipeline" / f"{pipeline_file}.yaml", config
        )

    trainer_file = ref.get("trainer_config_file")
    if trainer_file:
        _copy_template_with_substitution(
            tpl / "trainer" / f"{trainer_file}.yaml", dst / "trainer" / f"{trainer_file}.yaml", config
        )

    callbacks_file = ref.get("callbacks_file")
    if callbacks_file:
        _copy_template_with_substitution(
            tpl / "callbacks" / f"{callbacks_file}.yaml", dst / "callbacks" / f"{callbacks_file}.yaml", config
        )

    _copy_template_with_substitution(
        tpl / "tracker" / f"{config.tracker.value}.yaml", dst / "tracker" / f"{config.tracker.value}.yaml", config
    )

    # --- Train YAML (per-dataset template) ---
    _copy_template_with_substitution(tpl / f"train_{config.dataset.value}.yaml", dst / "train.yaml", config)
