...
- Some attention blocks, such as ``DotProductAttention``, also expose optional backend flags like ``attn_implementation`` to enable accelerated kernels when the hardware and dependency stack support them.
